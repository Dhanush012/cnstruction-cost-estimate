...
import { MaterialEstimator, MaterialService } from '../../material.service';
...
export class MaterialsComponent implements OnInit {
  materials: MaterialEstimator[] = [];
  pieChart: any;
  inputId: number | null=null;
  displayedColumns: string[] = ['resourceName', 'resourceQuantity', 'materialQuality', 'amount'];

  constructor(private materialService: MaterialService,private inputService:InputService) {}

  ngOnInit(): void {
    // const inputId = 21; 
    // this.fetchMaterials(inputId);
    this.inputId = this.inputService.getInputId();
    if (this.inputId) {
      this.fetchMaterials(this.inputId);
    } else {
      console.warn('No inputId found. Ensure user data is saved first.');
    }
      // if (id) {
      //   this.inputId = id;
  ...
  //   const savedId = this.inputService.getInputId();
  //   if (savedId) {
  //     this.inputId = savedId;
  //     this.fetchMaterials(savedId);
  //   } else {
  //     // Subscribe to service updates
  //     this.inputService.inputId$.subscribe((id) => {
  //       if (id !== null) {
  //         this.inputId = id;
  //         this.fetchMaterials(id);
  //       } else {
  ...
  fetchMaterials(inputId: number): void {
    this.materialService.getMaterialEstimates(inputId).subscribe({
      next: (data) => {
        this.materials = data;
        this.createPieChart();
      },
  ...
  createPieChart(): void {
    ...
    const ctx = document.getElementById('materialPieChart') as HTMLCanvasElement;
    this.pieChart = new Chart(ctx, {
      type: 'pie',
      data: {
        labels: this.materials.map((m) => m.resourceName),
        datasets: [
          {
            data: this.materials.map((m) => m.amount),
            backgroundColor: [
              '#FF6384', '#36A2EB', '#FFCE56', '#4CAF50', '#8E44AD',
              '#E74C3C', '#3498DB', '#F1C40F', '#2ECC71', '#16A085'
            ]
          }
        ]
      }
