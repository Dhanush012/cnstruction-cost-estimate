...
//    }
    ...
    public ResponseEntity<?> createAuthenticationToken(@RequestBody AuthenticationRequest authenticationRequest) throws Exception {
        // Validate role
        String requestedRole = authenticationRequest.getRole();
        if (!List.of("USER", "ADMIN", "CONSTRUCTOR").contains(requestedRole.toUpperCase())) {
        ...
        // Verify role matches
        if (!loginDetails.getRole().equalsIgnoreCase(requestedRole)) {
